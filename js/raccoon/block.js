Object.assign(window.Raccoon, {
    ...
    duplicateStack(startBlockId, positionOffset = {x: 20, y: 20}, returnRootId = false) { 
        ...
        newBlocks[newRootId].position.x += positionOffset.x; 
        newBlocks[newRootId].position.y += positionOffset.y; 
        
        Object.assign(sprite.blocks, newBlocks); 
        
        if (returnRootId) return newRootId;

        this.renderBlock(newBlocks[newRootId]); 
    ...
    deleteStack(startBlockId, spriteId) {
        ...
        if (parentCBlock) {
            const rootToUpdate = this.getStackRoot(parentCBlock.id, spriteId)?.id || parentCBlock.id;
        ...
        if (newBlockData.inputs && oldBlock.inputs) {
            for (const key in newBlockData.inputs) {
                if (oldBlock.inputs[key] !== undefined) {
                    ...
                    if (newBlockData.inputs[key].blockId) {
