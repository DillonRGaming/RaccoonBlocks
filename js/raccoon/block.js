Object.assign(window.Raccoon, {
    ...
    duplicateStack(startBlockId, mousePos, returnRootId = false, startDragging = false) { 
        ...
        this.updateBlockPositions(newRootId); // Pre-calculate layout
        ...
        if (newBlockData.inputs && oldBlock.inputs) {
            for (const key in newBlockData.inputs) {
                if (oldBlock.inputs[key] !== undefined) {
                    const oldShape = oldBlock.inputs[key].shape;
                    const newShape = newBlockData.inputs[key].shape;
                    const areShapesCompatible = (newShape === oldShape) || 
                                              (newShape === 'reporter' && ['reporter.leaf', 'reporter.square', 'boolean'].includes(oldShape)) ||
                                              (oldShape === 'reporter' && ['reporter.leaf', 'reporter.square', 'boolean'].includes(newShape));

                    if (areShapesCompatible) {
                        newBlockData.inputs[key].value = oldBlock.inputs[key].value;
                    }
                    
                    if (oldBlock.inputs[key].blockId && areShapesCompatible) {
                        newBlockData.inputs[key].blockId = oldBlock.inputs[key].blockId;
                        const childBlock = sprite.blocks[newBlockData.inputs[key].blockId];
